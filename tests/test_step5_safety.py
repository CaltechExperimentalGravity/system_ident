...
def test_snapshot_restore_round_trip():
    twin = _twin()
    snap = twin.snapshot_state(["C1:EXC_POS"])  # pre-run: no drive
    twin.inject("C1:EXC_POS", np.ones(int(4 * FS)), FS)
    assert np.any(twin.read(["C1:RESP_POS"], 4.0)["C1:RESP_POS"] != 0)

    twin.restore_state(snap)
    np.testing.assert_array_equal(twin.read(["C1:RESP_POS"], 4.0)["C1:RESP_POS"], 0.0)
...
def test_evaluate_flags_saturation_and_rms():
    ...
    segment = {
        "C1:EXC_POS": np.array([0.5, -2.0, 0.1]),   # peak 2.0 > 1.0 -> breach
        "C1:RESP_POS": np.full(100, 3.0),           # rms 3.0 > 1.0 -> breach
    }
    report = wd.evaluate(segment)
    assert not report.ok
    assert len(report.breaches) == 2
    assert report.drive_peaks["C1:EXC_POS"] == pytest.approx(2.0)
...
def test_evaluate_ok_within_limits():
    ...
    segment = {"C1:EXC_POS": np.array([0.5, -0.4]), "C1:RESP_POS": np.full(50, 0.2)}
    report = wd.evaluate(segment)
...
def test_check_aborts_and_hands_off_on_breach():
    ...
    wd.snapshot(["C1:EXC_POS"])  # capture quiet pre-run state

    twin.inject("C1:EXC_POS", np.full(int(8 * FS), 5.0), FS)  # over saturation
    segment = twin.read(["C1:EXC_POS", "C1:RESP_POS"], 8.0)
    ...
    np.testing.assert_array_equal(twin.read(["C1:RESP_POS"], 8.0)["C1:RESP_POS"], 0.0)


def test_operator_stop_ramps_and_restores():
    twin = _twin()
    wd = _watchdog(twin)
    wd.snapshot(["C1:EXC_POS"])
    twin.inject("C1:EXC_POS", np.ones(int(8 * FS)), FS)
