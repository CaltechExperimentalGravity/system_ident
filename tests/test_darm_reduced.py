...
def test_reduced_stages_carry_the_quad_modes():
    ...
    for m in QUAD_MODES_HZ:
        assert np.any(np.abs(peaks - m) < 0.03), f"quad mode {m} Hz not found in UIM stage; peaks={peaks}"


def test_absolute_scale_anchored_but_irrelevant():
    """Stages anchored to the placeholder magnitude at 100 Hz (κ semantics carry over); the
    absolute scale does not affect the loop because D = G/(A·C) is derived."""
    lr, ld = DARMLoop.default_reduced(), DARMLoop.default()
    f100 = np.array([100.0])
    for name in ("UIM", "PUM", "TST"):
        assert abs(lr.stage(name, f100)[0]) == pytest.approx(abs(ld.stage(name, f100)[0]), rel=1e-6)
...
def test_kappa_recovery_within_crb_in_smooth_band():
    ...
    for name, k_true in [("TST", 0.075), ("PUM", 0.42), ("UIM", 1.1)]:
...
def test_low_band_campaign_cannot_resolve_sharp_modes():
    ...
    khat_lo, sig_lo = tv.snapshot_kappa(biased, "UIM", 1.1, n_periods=16, seed=7)
