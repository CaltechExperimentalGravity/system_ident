...
def test_inject_applies_soft_start_stop_ramp():
    ...
    ramped.inject("X1:EXC_POS", drive, FS)
    mon = ramped.read(["X1:EXC_POS"], duration=30.0)["X1:EXC_POS"]
    ...
    plain.inject("X1:EXC_POS", drive, FS)
    assert plain.read(["X1:EXC_POS"], duration=30.0)["X1:EXC_POS"][0] == pytest.approx(1.0)
...
def test_inject_read_applies_plant_filter():
    ...
    twin.inject("X1:EXC_POS", drive, FS)

    out = twin.read(["X1:RESP_POS"], duration=8.0)["X1:RESP_POS"]
...
def test_impulse_response_matches_continuous_plant_at_low_freq():
    ...
    impulse = np.zeros(n)
    impulse[0] = 1.0
    twin.inject("X1:EXC_POS", impulse, FS)
    resp = twin.read(["X1:RESP_POS"], duration=dur)["X1:RESP_POS"]
...
def test_quiet_read_is_reproducible_noise():
    ...
    ya = a.read(["X1:RESP_POS"], duration=64.0)["X1:RESP_POS"]
    yb = b.read(["X1:RESP_POS"], duration=64.0)["X1:RESP_POS"]
...
def test_ramp_down_tapers_to_zero():
    ...
    twin.inject("X1:EXC_POS", drive, FS)
    twin.ramp_down("X1:EXC_POS", secs=2.0)

    monitored = twin.read(["X1:EXC_POS"], duration=10.0)["X1:EXC_POS"]
    n_ramp = int(2.0 * FS)
...
def test_inject_resamples_mismatched_rate():
    ...
    twin.inject("X1:EXC_POS", drive, 64.0)
    out = twin.read(["X1:RESP_POS"], duration=4.0)["X1:RESP_POS"]
...
def test_from_config_routes_channels_and_dofs_are_independent():
    ...
    config = {
        "channels": {
            "excitation": {"POS": "X1:EXC_1", "PIT": "X1:EXC_2"},
            "readback": {"POS": "X1:RESP_1", "PIT": "X1:RESP_2"},
        }
    }
    twin = TwinBackend.from_config(config, plant, fs=FS)
    assert twin.exc_channels == {"X1:EXC_1": "POS", "X1:EXC_2": "PIT"}

    # driving POS must not move PIT's readback
    drive = np.random.default_rng(0).standard_normal(int(4 * FS))
    twin.inject("X1:EXC_1", drive, FS)
    out = twin.read(["X1:RESP_1", "X1:RESP_2"], duration=4.0)
    assert np.any(out["X1:RESP_1"] != 0)
    np.testing.assert_array_equal(out["X1:RESP_2"], 0.0)


def test_unknown_channel_raises():
    twin = _single_dof_twin()
    with pytest.raises(KeyError):
        twin.inject("X1:NOPE", np.zeros(10), FS)
    with pytest.raises(KeyError):
        twin.read(["X1:NOPE"], duration=1.0)
...
def test_disturbance_asd_colors_quiet_readout():
    ...
    y_dist = twin_dist.read(["X1:RESP_POS"], duration=dur)["X1:RESP_POS"]
    ...
    y_sensor = twin_sensor.read(["X1:RESP_POS"], duration=dur)["X1:RESP_POS"]
